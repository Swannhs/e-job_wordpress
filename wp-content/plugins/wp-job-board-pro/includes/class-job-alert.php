...
class WP_Job_Board_Pro_Job_Alert {
	...
	public static function process_add_job_alert() {
        ...
        if ( $alert_id ) {
	        ...
	        $return = array( 'status' => true, 'msg' => esc_html__('Add job alert successfully.', 'wp-job-board-pro') );
		   	echo wp_json_encode($return);
		   	exit;
	    } else {
			$return = array( 'status' => false, 'msg' => esc_html__('Add job alert error.', 'wp-job-board-pro') );
		   	echo wp_json_encode($return);
		   	exit;
		}
	}
